...
export default function Home() {
  ...
  useEffect(() => {
    const getConnections = async () => {
      ...
      if (user) {
        const obj = await db.readRecordFromTable(
          "accounts",
          "accountId",
          `${user.id}`
        );
  ...
  const filteredAccounts = accounts.filter((account) => {
    ...
    const nameMatches =
      `${account.firstName.toLowerCase()} ${account.lastName.toLowerCase()}`.startsWith(
        search.toLowerCase()
      );
    ...
    const studentData = students.find(
      (student) => student.studentId === account.accountId
    );
    const alumniData = alumni.find(
      (alumnus) => alumnus.alumniId === account.accountId
    );
    const majorMatches = studentData
      ? selectedMajor
        ? studentData.major.startsWith(selectedMajor)
        : true
      : selectedMajor == "";
    const fieldMatches = alumniData
      ? selectedField
        ? alumniData.currentField
            .toLowerCase()
            .startsWith(selectedField.toLowerCase())
        : true
      : selectedField == "";
    const titleMatches = alumniData
      ? selectedTitle
        ? alumniData.currentJobTitle
            .toLowerCase()
            .startsWith(selectedTitle.toLowerCase())
        : true
      : selectedTitle == "";
    const companyMatches = alumniData
      ? selectedCompany
        ? alumniData.currentCompany
            .toLowerCase()
            .startsWith(selectedCompany.toLowerCase())
        : true
      : selectedCompany == "";
    const notConnection = showConnections
      ? true
      : !connections.includes(account.accountId);
    return (
      notYou &&
      nameMatches &&
      accountTypeMatches &&
        ...
        <div className="filter">
          ...
          <div style={{ marginTop: "15px", fontSize: "17px" }}>
            ...
            <input
              ...
              name="placeholder"
              value="placeholder"
              onChange={handleConnectionsChecked}
              defaultChecked
            />
          </div>
        </div>
